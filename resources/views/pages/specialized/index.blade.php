...
<div class="container">
        ...
        <h2>Specialized Records</h2>

        <a href="{{ route('specialized.create') }}" class="btn btn-primary">Create New</a>
...
<tr>
        ...
        <td>{{ $item->id }}</td>
                    ...
                    @php
                        // Get file extension
                        $fileExtension = pathinfo($item->title, PATHINFO_EXTENSION);
                    ...
                    @if($isImage)
                        <td><img src="{{ $item->title }}" style="height: 50px; object-fit: cover;" /></td>
                    @elseif($isVideo)
                        <td>
                            <video width="100" height="50" controls>
                                <source src="{{ $item->title }}" type="video/{{ $fileExtension }}">
                                Your browser does not support the video tag.
                            </video>
                        </td>
                    @else
...
<td>
        ...
        <a href="{{ route('specialized.edit', $item->id) }}" class="btn btn-info">Edit</a>
            ...
            $('body').on('click', '.deleteItem', function () {
                ...
                swal({
                    ...
                    if (willDelete) {
                        $.ajax({
                            type: "DELETE",
                            url: "{{ url('admin/specialized') }}" + '/' + item_id,
